    ...
    def run(self, targets):
        ...
        for target in targets:
            output_file = os.path.join(
                self.raw_output_dir, f"k6_{target.replace('/', '_')}.json"
            )
            script_content = f"""
...
"""
            script_path = os.path.join(
                self.output_dir, f"load_test_{target.replace('/', '_')}.js"
            )
            with open(script_path, "w") as f:
                f.write(script_content)

            cmd = ["k6", "run", "--out", f"json={output_file}", script_path]
    ...
    def parse_results(self, data):
        ...
        normalized = []
        for item in data:
            normalized.append({"target": item["target"], "rps": 0, "p95_latency": 0})
        if self.dm:
